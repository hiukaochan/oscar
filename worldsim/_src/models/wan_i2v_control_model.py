...
from worldsim._src.models.wan_t2v_model import DataType, T2VCondition, T2VModelConfig, WANDiffusionModel


WAN2PT1_I2V_COND_LATENT_KEY = "i2v_WAN2PT1_cond_latents"


class I2VWan2pt1ControlModel(WANDiffusionModel):
    ...
    def get_data_and_condition(self, data_batch: dict[str, torch.Tensor]) -> Tuple[Tensor, T2VCondition]:
        ...
        if WAN2PT1_I2V_COND_LATENT_KEY not in data_batch:
            ...
            latent_control_input = [data_batch[WAN2PT1_I2V_COND_LATENT_KEY]]
            hint_key = 'hint_key'
            latent_control_input.append(self.encode(data_batch[hint_key]).contiguous().to(**self.tensor_kwargs))

            latent_control_input = torch.cat(latent_control_input, dim=1)
            data_batch[WAN2PT1_I2V_COND_LATENT_KEY] = latent_control_input
