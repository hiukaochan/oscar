...
try:
    ...
    def flash_apply_rotary_emb(x, cos, sin, interleaved=True, inplace=False):
        """Pure PyTorch fallback for flash_attn apply_rotary_emb (interleaved=True)."""
        # x: [batch, seq, nheads, head_dim] or [seq, nheads, head_dim]
        # cos, sin: [seq, head_dim//2]
        ndim = x.dim()
        # broadcast cos/sin to x shape: insert batch and nheads dims
        view = [1] * ndim
        view[-3] = cos.shape[-2]   # seq dim
        view[-1] = cos.shape[-1]   # head_dim//2
        cos = cos.view(view)
        sin = sin.view(view)
        x_even = x[..., 0::2]
        x_odd  = x[..., 1::2]
    ...
    print("flash_attn is not installed; using pure-PyTorch rotary fallback.")
...
from worldsim._src.modules.selective_activation_checkpoint import (
    CheckpointMode,
)
from worldsim._src.modules.selective_activation_checkpoint import SACConfig as SACConfig

from worldsim._src.utils.context_parallel import split_inputs_cp
...
class WanModel(WeightTrainingStat):
    ...
    def fully_shard(self, mesh, **fsdp_kwargs):
        ...
        fully_shard(self.head, mesh=mesh, reshard_after_forward=True, **fsdp_kwargs)
