...
from worldsim._src.reason1.configs.default.model_config import FSDP2ModelConfig
from worldsim._src.reason1.models.vlm_base import VLMBaseModel, init_mesh
from worldsim._src.reason1.networks.qwen2_5_vl import Qwen2_5_VisionTransformerPretrainedModel, Qwen2_5_VLModel
from worldsim._src.reason1.networks.qwen2_5_vl import get_rope_index as get_rope_index_v2_5
from worldsim._src.reason1.networks.qwen2_vl import Qwen2VisionTransformerPretrainedModel, Qwen2VLModel
from worldsim._src.reason1.networks.qwen2_vl import get_rope_index as get_rope_index_v2
from worldsim._src.reason1.parallelisms.optimizer import build_lr_schedulers, build_optimizers
from worldsim._src.reason1.parallelisms.parallelize_qwen import parallelize_qwen
from worldsim._src.reason1.tokenizer.processor import Processor
...
class QwenModel(VLMBaseModel):
    ...
    def build_model(self, model_config):
        text_attn_impl = getattr(model_config, "_attn_implementation", "sdpa")
        vision_attn_impl = text_attn_impl
        if hasattr(model_config, "vision_config") and model_config.vision_config is not None:
            vision_attn_impl = getattr(model_config.vision_config, "_attn_implementation", text_attn_impl)
        model_config._attn_implementation = text_attn_impl
        if hasattr(model_config, "vision_config") and model_config.vision_config is not None:
            model_config.vision_config._attn_implementation = vision_attn_impl
        log.info(
            "Using Reason1 attention implementation: "
            f"text={text_attn_impl}, vision={vision_attn_impl}"
        )
        ...
        if model_config.model_type == "qwen2_5_vl":
            ...
            config = AutoConfig.from_pretrained(
                model_config.name_or_path, torch_dtype=torch.bfloat16, attn_implementation=text_attn_impl
            )
