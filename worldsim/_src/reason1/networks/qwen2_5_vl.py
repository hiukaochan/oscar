...
try:
    ...
    # transformers >= 5.x removed SlidingWindowCache; use DynamicCache as compat shim
    SlidingWindowCache = DynamicCache
from transformers.modeling_attn_mask_utils import AttentionMaskConverter
from transformers.modeling_outputs import BaseModelOutputWithPast, ModelOutput
from transformers.modeling_rope_utils import ROPE_INIT_FUNCTIONS

# transformers >= 5.x removed the 'default' RoPE type; add it back as standard RoPE
...
if not is_flash_attn_2_available():
    # Soft-warn: the cosmos2 inference path uses cached t5 embeddings and never
    # calls the qwen text encoder, so the underlying flash_attn dep is dead code
    # at inference time. The hard-assert here blocks all import-time chains.
    import warnings as _w
    _w.warn("flash_attn_2 not available; qwen2_5_vl import allowed in soft mode")
