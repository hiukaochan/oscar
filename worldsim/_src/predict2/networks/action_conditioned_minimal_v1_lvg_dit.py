...
from worldsim._src.predict2.networks.minimal_v4_dit import MiniTrainDIT
...
def post_init_zero_action_embedder(net: "ActionConditionedMinimalV1LVGDiT") -> None:
    """Zero-init the second linear layer of both action embedders.

    Called by the net factory after instantiation (and before ckpt load), so
    action_emb ≡ 0 at iter 0 and the forward pass is identical to the base
    (non-action) checkpoint.
    """
    import torch.nn as nn
    nn.init.zeros_(net.action_embedder_B_D.fc2.weight)
    nn.init.zeros_(net.action_embedder_B_D.fc2.bias)
    nn.init.zeros_(net.action_embedder_B_3D.fc2.weight)
    nn.init.zeros_(net.action_embedder_B_3D.fc2.bias)
