...
from worldsim._ext.imaginaire.lazy_config import instantiate as lazy_instantiate
from worldsim._ext.imaginaire.model import ImaginaireModel
from worldsim._ext.imaginaire.utils import log, misc
from worldsim._ext.imaginaire.utils.checkpointer import non_strict_load_model
from worldsim._src.utils.context_parallel import (
    broadcast,
    broadcast_split_tensor,
    cat_outputs_cp,
    find_split,
)
from worldsim._ext.common.utils.count_params import count_params
from worldsim._ext.imaginaire.utils.ema import FastEmaModelUpdater
from worldsim._ext.common.utils.fsdp_helper import hsdp_device_mesh
from worldsim._ext.common.utils.optim_instantiate import get_base_scheduler
from worldsim._src.modules.conditioner import DataType, T2VCondition
from worldsim._src.datasets.utils import VIDEO_RES_SIZE_INFO
from worldsim._ext.callbacks.model_weights_stats import WeightTrainingStat
from worldsim._src.utils.context_parallel import (
    broadcast,
    broadcast_split_tensor,
    cat_outputs_cp,
)
from worldsim._src.models.fm_solvers_unipc import FlowUniPCMultistepScheduler
from worldsim._src.schedulers.rectified_flow import RectifiedFlow
from worldsim._src.utils.dtensor_helper import DTensorFastEmaModelUpdater, broadcast_dtensor_model_states
from worldsim._src.tokenizers.base_vae import BaseVAE
from worldsim._src.predict2.text_encoders.text_encoder import TextEncoder, TextEncoderConfig
from worldsim._src.models.wan_t2v_model import EMAConfig # Import shared EMAConfig to avoid Hydra type conflicts
...
class Text2WorldModelRectifiedFlow(ImaginaireModel):
    ...
    def forward(self, data_batch: dict[str, torch.Tensor]) -> tuple[dict[str, torch.Tensor], torch.Tensor]:
        ...
        T_latent = vt_pred_B_C_T_H_W.shape[2]
        if frame_valid_mask is not None and frame_valid_mask.shape[1] == T_latent:
            # Mask out padded (freeze-frame) latent frames from loss
            mask = frame_valid_mask.to(vt_pred_B_C_T_H_W.device)
            mask_B_1_T_1_1 = mask[:, None, :, None, None]
            C, H, W = vt_pred_B_C_T_H_W.shape[1], vt_pred_B_C_T_H_W.shape[3], vt_pred_B_C_T_H_W.shape[4]
            per_element_loss = (vt_pred_B_C_T_H_W - vt_B_C_T_H_W) ** 2
            per_instance_loss = (per_element_loss * mask_B_1_T_1_1).sum(dim=[1, 2, 3, 4]) / (
    ...
    def load_state_dict(self, state_dict: Mapping[str, Any], strict: bool = False, assign: bool = False):  # noqa: F821
        """
