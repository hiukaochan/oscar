...
from  worldsim._ext.imaginaire.utils.distributed import broadcast, get_rank, sync_model_states
from  worldsim._ext.imaginaire.utils.easy_io import easy_io
from worldsim._src.tokenizers.interface import VideoTokenizerInterface
...
class Wan2pt1VAEInterface(VideoTokenizerInterface):
    def __init__(self, chunk_duration: int = 81, load_mean_std=False, **kwargs):
        import os as _os
        _default_vae = _os.environ.get("WAN_VAE_PATH")
        if not _default_vae or not _os.path.exists(_default_vae):
            from huggingface_hub import hf_hub_download
            _default_vae = hf_hub_download(
        ...
        self.model = WanVAE(
            ...
            is_amp=False,
            load_mean_std=load_mean_std,
            vae_pth=kwargs.get("vae_pth", _default_vae),
