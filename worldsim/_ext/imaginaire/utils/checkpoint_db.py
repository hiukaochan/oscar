...
from worldsim._ext.imaginaire.flags import EXPERIMENTAL_CHECKPOINTS, INTERNAL
...
def get_checkpoint_path(checkpoint_uri: str) -> str:
    """Return checkpoint path for S3 URI, HF Hub repo id, or local path."""
    if INTERNAL:
    ...
    if os.path.exists(checkpoint_uri):
        return checkpoint_uri
    # Treat repo-id-shaped strings (e.g. "nvidia/Cosmos-Reason1-7B") as HF Hub
    # references — snapshot_download fetches the full repo and returns the
    # local cache path. Enables the public OSCAR release to pull text-encoder
    # / VAE weights from HF without bundling internal NFS paths.
    if "/" in checkpoint_uri and checkpoint_uri.count("/") <= 2:
        from huggingface_hub import snapshot_download
        return snapshot_download(repo_id=checkpoint_uri)
